...
| `CALL src=2` (helper by BTF ID) | Partial | `src/ir/unmarshal.cpp`: `makeJmp` (`CallBtf`); `src/ir/kfunc.cpp`: `make_kfunc_call`; `src/ir/cfg_builder.cpp`: `check_instruction_feature_support`, `instruction_seq_to_cfg` (table-driven subset; unknown IDs and unavailable program-type/privileged constraints are rejected; return contracts currently limited to integer and map-value-or-null style) |
