...
static void require_subsumes(const TypeRestrictions& specific_type,
                             const std::vector<LinearConstraint>& specific_csts,
                             const TypeRestrictions& general_type,
...
static void require_not_subsumes(const TypeRestrictions& a_type,
                                 const std::vector<LinearConstraint>& a_csts,
                                 const TypeRestrictions& b_type,
...
TEST_CASE("Straightforward value and type subsumption", "[subsumption][lattice]") {
    ...
    require_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {},
    ...
    require_subsumes({{r0_type, TS::singleton(T_NUM)}, {r1_type, TS::singleton(T_NUM)}},
                     ...
                     {eq(r0.svalue, r1.svalue)});
    require_not_subsumes({{r0_type, TS::singleton(T_NUM)}, {r1_type, TS::singleton(T_NUM)}},
                         {eq(r0.svalue, r1.svalue)},
                         {{r0_type, TS::singleton(T_NUM)}, {r1_type, TS::singleton(T_NUM)}},
                         {r0.svalue >= 0});
}
...
TEST_CASE("Subsumption of unrelated or disjoint domains", "[subsumption][lattice]") {
    using namespace dsl_syntax;
    require_not_subsumes({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 1},
                         {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 2});
    require_not_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {},
...
TEST_CASE("Subsumption with absent type-specific variables", "[subsumption][lattice]") {
    ...
    require_subsumes({{r0_type, TS::singleton(T_NUM)}}, {},
                     {{r0_type, TS::of({T_NUM, T_PACKET})}}, {r0.packet_offset == 5});
...
TEST_CASE("Subsumption with present type-specific variables", "[subsumption][lattice]") {
    ...
    require_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset == 8},
                     type_general_domain, general_domain);
    // The reverse is not true, as `general_domain` has a non-bottom `stack_offset`.
    require_not_subsumes(type_general_domain, general_domain,
                         {{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset == 8});
}
