...
TEST_CASE("Basic subsumption properties", "[subsumption][lattice]") {
    ...
    require_subsumes({{r0_type, TS::singleton(T_NUM)}}, {}, {{r0_type, TS::singleton(T_NUM)}}, {});

    // Anything is subsumed by top.
    require_subsumes({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 5}, {}, {});

    // Top is not subsumed by anything but top.
    require_not_subsumes({}, {}, {{r0_type, TS::singleton(T_NUM)}}, {});
}
...
TEST_CASE("Straightforward value and type subsumption", "[subsumption][lattice]") {
    ...
    require_subsumes({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 5}, {{r0_type, TS::singleton(T_NUM)}},
                     {r0.svalue >= 0, r0.svalue <= 10});
    require_not_subsumes({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 0, r0.svalue <= 10},
                         {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 5});

    // A sub-range is subsumed by a super-range.
    require_subsumes({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 2, r0.svalue <= 8},
                     {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 0, r0.svalue <= 10});

    // A specific type is subsumed by a type range containing it.
    require_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {}, {{r0_type, TS::of({T_NUM, T_CTX, T_PACKET, T_STACK})}},
                     {});
    require_not_subsumes({{r0_type, TS::of({T_NUM, T_CTX, T_PACKET, T_STACK})}}, {},
                         {{r0_type, TS::singleton(T_PACKET)}}, {});

    // Relational constraints must also hold.
    require_subsumes({{r0_type, TS::singleton(T_NUM)}, {r1_type, TS::singleton(T_NUM)}},
                     {eq(r0.svalue, r1.svalue), r0.svalue == 5},
                     {{r0_type, TS::singleton(T_NUM)}, {r1_type, TS::singleton(T_NUM)}}, {eq(r0.svalue, r1.svalue)});
    require_not_subsumes({{r0_type, TS::singleton(T_NUM)}, {r1_type, TS::singleton(T_NUM)}}, {eq(r0.svalue, r1.svalue)},
                         {{r0_type, TS::singleton(T_NUM)}, {r1_type, TS::singleton(T_NUM)}}, {r0.svalue >= 0});
}

TEST_CASE("Subsumption of unrelated or disjoint domains", "[subsumption][lattice]") {
    using namespace dsl_syntax;
    require_not_subsumes({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 1}, {{r0_type, TS::singleton(T_NUM)}},
                         {r0.svalue == 2});
    require_not_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {}, {{r0_type, TS::singleton(T_STACK)}}, {});
    require_not_subsumes({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 0, r0.svalue <= 5},
                         {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 3, r0.svalue <= 8});
}
...
TEST_CASE("Subsumption with absent type-specific variables", "[subsumption][lattice]") {
    ...
    require_subsumes({{r0_type, TS::singleton(T_NUM)}}, {}, {{r0_type, TS::of({T_NUM, T_PACKET})}},
                     {r0.packet_offset == 5});
    ...
    require_not_subsumes({{r0_type, TS::of({T_NUM, T_PACKET})}}, {r0.packet_offset == 5},
                         {{r0_type, TS::singleton(T_NUM)}}, {});

    // Same test with stack-related variables.
    require_subsumes({{r0_type, TS::singleton(T_NUM)}}, {}, {{r0_type, TS::of({T_NUM, T_CTX, T_PACKET, T_STACK})}},
                     {r0.stack_offset == 128, r0.stack_numeric_size == 8});
    require_not_subsumes({{r0_type, TS::of({T_NUM, T_CTX, T_PACKET, T_STACK})}},
                         {r0.stack_offset == 128, r0.stack_numeric_size == 8}, {{r0_type, TS::singleton(T_NUM)}}, {});
}
...
TEST_CASE("Subsumption with present type-specific variables", "[subsumption][lattice]") {
    ...
    require_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset == 8},
                     {{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset >= 0, r0.packet_offset <= 16});
    require_not_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset >= 0, r0.packet_offset <= 16},
                         {{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset == 8});

    // Check with multiple active type-specific variables.
    const TypeRestrictions type_general_domain{{r0_type, TS::of({T_PACKET, T_STACK})}};
    ...
    require_subsumes({{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset == 8}, type_general_domain,
                     general_domain);
    // The reverse is not true, as `general_domain` has a non-bottom `stack_offset`.
    require_not_subsumes(type_general_domain, general_domain, {{r0_type, TS::singleton(T_PACKET)}},
                         {r0.packet_offset == 8});
}
...
TEST_CASE("Edge case subsumption with bottom and mixed types", "[subsumption][lattice]") {
    ...
    require_subsumes({}, {r0.svalue > 0, r0.svalue < 0}, {{r0_type, TS::singleton(T_NUM)}}, {});
    require_subsumes({}, {r0.svalue > 0, r0.svalue < 0}, {}, {});

    // Nothing (except bottom) is subsumed by bottom.
    require_not_subsumes({{r0_type, TS::singleton(T_NUM)}}, {}, {}, {r0.svalue > 0, r0.svalue < 0});

    // A single, specific type should be subsumed by a wider type range.
    require_subsumes({{r0_type, TS::singleton(T_SHARED)}}, {r0.shared_offset == 16},
                     {{r0_type, TS::of({T_NUM, T_CTX, T_PACKET, T_STACK, T_SHARED})}}, {});

    // A wider type range is not subsumed by a single type, even if the offset is unconstrained.
    require_not_subsumes({{r0_type, TS::of({T_NUM, T_CTX, T_PACKET, T_STACK, T_SHARED})}}, {},
