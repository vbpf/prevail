...
TEST_CASE("Straightforward value and type subsumption", "[subsumption][lattice]") {
    ...
    require_not_subsumes({r0.type == T_NUM}, {r0.svalue >= 0, r0.svalue <= 10},
                         {
                             r0.type == T_NUM,
                         },
                         {r0.svalue == 5});
    ...
    require_subsumes({r0.type == T_NUM}, {r0.svalue >= 2, r0.svalue <= 8}, {r0.type == T_NUM},
...
TEST_CASE("Subsumption of unrelated or disjoint domains", "[subsumption][lattice]") {
    ...
    require_not_subsumes({r0.type == T_NUM}, {r0.svalue >= 0, r0.svalue <= 5}, {r0.type == T_NUM},
...
TEST_CASE("Subsumption with absent type-specific variables", "[subsumption][lattice]") {
    ...
    require_subsumes({r0.type == T_NUM}, {}, {r0.type >= T_NUM, r0.type <= T_STACK},
...
TEST_CASE("Subsumption with present type-specific variables", "[subsumption][lattice]") {
    ...
    require_subsumes({r0.type == T_PACKET}, {r0.packet_offset == 8}, {r0.type == T_PACKET},
                     {r0.packet_offset >= 0, r0.packet_offset <= 16});
    require_not_subsumes({r0.type == T_PACKET}, {r0.packet_offset >= 0, r0.packet_offset <= 16}, {r0.type == T_PACKET},
                         {r0.packet_offset == 8});

    // Check with multiple active type-specific variables.
    const auto type_general_domain = {r0.type >= T_PACKET, r0.type <= T_STACK};
    ...
    require_subsumes({r0.type == T_PACKET}, {r0.packet_offset == 8}, type_general_domain, general_domain);
    // The reverse is not true, as `general_domain` has a non-bottom `stack_offset`.
    require_not_subsumes(type_general_domain, general_domain, {r0.type == T_PACKET}, {r0.packet_offset == 8});
}
