...
TEST_CASE("unmarshal external helper call", "[marshal]") {
    RawProgram program{
        .prog =
            {
                {
                    EbpfInst{.opcode = INST_OP_CALL, .src = INST_CALL_STATIC_HELPER, .imm = -1},
                    EbpfInst{.opcode = INST_OP_EXIT},
                },
            },
        .info =
            {
        ...
        .relocations = {{0, "bpf_get_prandom_u32"}},
    };

    const InstructionSeq inst_seq = std::get<InstructionSeq>(unmarshal(program));

    REQUIRE(inst_seq.size() == 2);
    auto [_, call_inst, _2] = inst_seq.front();
    (void)_;  // unused
    (void)_2; // unused
    // REQUIRE that call_inst is a Call object.
    REQUIRE(std::holds_alternative<Call>(call_inst));
    REQUIRE(std::get<Call>(call_inst).name == "get_prandom_u32");

    // Negative test. Unknown helper.
    program.relocations = {{0, "unknown_helper"}};

    auto result = unmarshal(program);
    auto* error_message = std::get_if<std::string>(&result);
    REQUIRE(error_message != nullptr);
    REQUIRE(*error_message == "0: invalid helper function name unknown_helper\n");

    // Negative test. No relocation entry.
    program.relocations = {};

    result = unmarshal(program);
    error_message = std::get_if<std::string>(&result);
    REQUIRE(error_message != nullptr);
    REQUIRE(*error_message == "0: call by name not resolved at 0\n");
}