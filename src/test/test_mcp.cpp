...
struct McpTestHarness {
    prevail_mcp::PrevailPlatformOps ops;
    prevail_mcp::AnalysisEngine engine;
...
TEST_CASE("label_to_json serializes correctly", "[mcp][serializer]") {
    SECTION("simple label") {
        Label label{5, -1, {}};
        auto j = prevail_mcp::label_to_json(label);
        REQUIRE(j["from"] == 5);
        REQUIRE(j["to"] == -1);
    ...
    SECTION("exit label") {
        auto j = prevail_mcp::label_to_json(Label::exit);
...
TEST_CASE("invariant_to_json serializes correctly", "[mcp][serializer]") {
    SECTION("bottom invariant") {
        auto j = prevail_mcp::invariant_to_json(StringInvariant::bottom());
        REQUIRE(j.is_array());
        REQUIRE(j.size() == 1);
        REQUIRE(j[0] == "_|_");
    }
    ...
    SECTION("top invariant") {
        auto j = prevail_mcp::invariant_to_json(StringInvariant::top());
        REQUIRE(j.is_array());
        REQUIRE(j.empty());
    }
    ...
    SECTION("non-empty invariant") {
        ...
        auto j = prevail_mcp::invariant_to_json(StringInvariant{constraints});
        REQUIRE(j.is_array());
        REQUIRE(j.size() == 2);
    }
}
...
TEST_CASE("interval_to_json serializes correctly", "[mcp][serializer]") {
    auto j = prevail_mcp::interval_to_json(Interval::top());
...
TEST_CASE("line_info_to_json serializes correctly", "[mcp][serializer]") {
    ...
    auto j = prevail_mcp::line_info_to_json(info);
...
TEST_CASE("error_to_json serializes correctly", "[mcp][serializer]") {
    SECTION("error with location") {
        ...
        err.where = Label{7, -1, {}};
        auto j = prevail_mcp::error_to_json(err);
...
TEST_CASE("instruction_to_json produces text field", "[mcp][serializer]") {
    ...
    auto j = prevail_mcp::instruction_to_json(inst);
    REQUIRE(j.contains("text"));
    REQUIRE(j["text"].is_string());
...
TEST_CASE("assertion_to_json produces text field", "[mcp][serializer]") {
    ...
    auto j = prevail_mcp::assertion_to_json(assertion);
    REQUIRE(j.contains("text"));
    REQUIRE(j["text"].is_string());
    REQUIRE_FALSE(j["text"].get<std::string>().empty());
}
...
TEST_CASE("engine reuses session for same program", "[mcp][engine]") {
    ...
    prevail_mcp::PrevailPlatformOps ops(&g_ebpf_platform_linux);
...
TEST_CASE("transport: NDJSON read and write", "[mcp][transport]") {
    ...
    prevail_mcp::McpTransport transport(out);
