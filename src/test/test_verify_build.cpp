...
// this memory-access proof. Diagnostic: 3: Upper bound must be at most EBPF_TOTAL_STACK_SIZE (valid_access(r1.offset,
// width=r2) for write)
TEST_SECTION_FAIL(
...
// control-flow pattern. Diagnostic: 4: Invalid type (r1.type in {number, ctx, stack, packet, shared})
...
// skipped (ElfSubprogramResolution): Known loader limitation: subprogram resolution/disambiguation is incomplete for
// this object. Diagnostic: Subprogram not found: ebpf_map_update_elem
TEST_SECTION_SKIP("build", "badrelo.o", ".text", verify_test::VerifyIssueKind::ElfSubprogramResolution,
                  "Known loader limitation: subprogram resolution/disambiguation is incomplete for this object. "
                  "Diagnostic: Subprogram not found: ebpf_map_update_elem")
...
// conservative here. Diagnostic: 6: Illegal map update with a non-numerical value [4092-4096) (within(r2:key_size(r1)))
...
// expected failure (VerifierTypeTracking): Known verifier limitation: register type refinement is too imprecise in this
// control-flow pattern. Diagnostic: 8: Invalid type (r2.type == func)
...
// expected failure (VerifierTypeTracking): Known verifier limitation: register type refinement is too imprecise in this
// control-flow pattern. Diagnostic: 16: Invalid type (r7.type in {ctx, stack, packet, shared})
...
// coarse for this access path. Diagnostic: 52: Stack content is not numeric (valid_access(r3.offset, width=r4) for
// read)
...
// expected failure (VerifierContextModeling): Known verifier limitation: context-offset modeling is too restrictive for
// this access pattern. Diagnostic: 8: Nonzero context offset (r1.ctx_offset == 0)
...
// expected failure (VerifierBoundsTracking): Known verifier limitation: interval/bounds refinement loses precision for
// this memory-access proof. Diagnostic: 10: Upper bound must be at most packet_size (valid_access(r1.offset, width=4)
// for read)
...
// expected failure (VerifierMapTyping): Known verifier limitation: map value/key typing and scalarization are too
// conservative here. Diagnostic: 10: Illegal map update with a non-numerical value [4088-4096)
// (within(r3:value_size(r1)))
TEST_SECTION_FAIL(
...
// skipped (ElfSubprogramResolution): Known loader limitation: subprogram resolution/disambiguation is incomplete for
// this object. Diagnostic: Subprogram not found: bpf_map_lookup_elem
TEST_SECTION_SKIP("build", "externalfunction.o", ".text", verify_test::VerifyIssueKind::ElfSubprogramResolution,
                  "Known loader limitation: subprogram resolution/disambiguation is incomplete for this object. "
                  "Diagnostic: Subprogram not found: bpf_map_lookup_elem")
// expected failure (VerifierTypeTracking): Known verifier limitation: register type refinement is too imprecise in this
// control-flow pattern. Diagnostic: 1: Invalid type (r3.type in {number, ctx, stack, packet, shared})
...
// expected failure (VerifierMapTyping): Known verifier limitation: map value/key typing and scalarization are too
// conservative here. Diagnostic: 5: Illegal map update with a non-numerical value [4092-4096) (within(r2:key_size(r1)))
...
// expected failure (VerifierBoundsTracking): Known verifier limitation: interval/bounds refinement loses precision for
// this memory-access proof. Diagnostic: 19: Upper bound must be at most r1.shared_region_size (valid_access(r1.offset,
// width=4) for write)
TEST_SECTION_FAIL(
...
// expected failure (VerifierBoundsTracking): Known verifier limitation: interval/bounds refinement loses precision for
// this memory-access proof. Diagnostic: 15: Upper bound must be at most EBPF_TOTAL_STACK_SIZE (valid_access(r5.offset,
// width=1) for write)
TEST_SECTION_FAIL(
...
// expected failure (VerifierBoundsTracking): Known verifier limitation: interval/bounds refinement loses precision for
// this memory-access proof. Diagnostic: 10: Upper bound must be at most r1.shared_region_size (valid_access(r1.offset,
// width=8) for read)
TEST_SECTION_FAIL(
...
// path. Diagnostic: 7: Possible null access (valid_access(r0.offset, width=4) for write)
...
// expected failure (VerifierBoundsTracking): Known verifier limitation: interval/bounds refinement loses precision for
// this memory-access proof. Diagnostic: 4: Upper bound must be at most packet_size (valid_access(r2.offset, width=4)
// for read)
...
// expected failure (VerifierTypeTracking): Known verifier limitation: register type refinement is too imprecise in this
// control-flow pattern. Diagnostic: 13: Invalid type (r7.type in {ctx, stack, packet, shared})
...
// expected failure (VerifierStackInitialization): Known verifier limitation: stack initialization tracking is too
// coarse for this access path. Diagnostic: 12: Stack content is not numeric (valid_access(r4.offset, width=r5) for
// read)
...
// expected failure (VerifierPointerArithmetic): Known verifier limitation: pointer-arithmetic typing is too restrictive
// in this pattern. Diagnostic: 2: Only numbers can be added to pointers (r2.type in {ctx, stack, packet, shared} ->
// r1.type == number)
TEST_SECTION_FAIL(
...
// expected failure (VerifierTypeTracking): Known verifier limitation: register type refinement is too imprecise in this
// control-flow pattern. Diagnostic: 30: Invalid type (r0.type == number)
...
// expected failure (VerifierTypeTracking): Known verifier limitation: register type refinement is too imprecise in this
// control-flow pattern. Diagnostic: 19: Invalid type (r1.type == map_fd)
...
// expected failure (VerifierStackInitialization): Known verifier limitation: stack initialization tracking is too
// coarse for this access path. Diagnostic: 6: Stack content is not numeric (valid_access(r2.offset, width=r3) for read)
...
// expected failure (VerifierTypeTracking): Known verifier limitation: register type refinement is too imprecise in this
// control-flow pattern. Diagnostic: 3: Invalid type (r2.type == map_fd_programs)
...
// skipped (PlatformHelperAvailability): Known platform-model limitation: helper availability differs from the sample
// expectation. Diagnostic: error: rejected: helper function is unavailable on this platform (at 7)
