...
TEST_CASE("EbpfDomain disjoint value join", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 0}, {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 5},
                 {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 0, r0.svalue <= 5});
}

TEST_CASE("join respects partial order", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 1}, {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 2},
                 {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 1, r0.svalue <= 2});
}

// 2) Value constraints with same type
TEST_CASE("join keeps equal constants", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 7}, {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 7},
                 {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 7});
}

TEST_CASE("join convex hull of disjoint constants", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 1}, {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 3},
                 {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 1, r0.svalue <= 3});
}

TEST_CASE("join overlapping intervals", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 0, r0.svalue <= 5},
                 {{r0_type, TS::singleton(T_NUM)}}, {r0.svalue >= 3, r0.svalue <= 10},
...
TEST_CASE("join widens type and preserves offset from one side", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset == 8},
                 {{r0_type, TS::singleton(T_NUM)}}, {},
...
TEST_CASE("join with matching map types", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_MAP)}}, {r0.svalue == 1},
                 {{r0_type, TS::singleton(T_MAP)}}, {r0.svalue == 3},
                 {{r0_type, TS::singleton(T_MAP)}}, {r0.svalue >= 1, r0.svalue <= 3});
}
...
TEST_CASE("join with different types and unknown offsets", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_MAP)}}, {r0.svalue == 1},
                 {{r0_type, TS::singleton(T_STACK)}}, {r0.svalue == 2},
                 {{r0_type, TS::of({T_MAP, T_NUM, T_CTX, T_PACKET, T_STACK})}},
...
TEST_CASE("join preserves shared offset across type widening", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_SHARED)}}, {r0.shared_offset == 0},
                 {{r0_type, TS::singleton(T_NUM)}}, {},
...
TEST_CASE("join context offsets", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_CTX)}}, {r0.ctx_offset == 0},
                 {{r0_type, TS::singleton(T_CTX)}}, {r0.ctx_offset == 16},
                 {{r0_type, TS::singleton(T_CTX)}}, {r0.ctx_offset >= 0, r0.ctx_offset <= 16});
}

TEST_CASE("join map fd constants", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_MAP)}}, {r0.map_fd == 10},
                 {{r0_type, TS::singleton(T_MAP)}}, {r0.map_fd == 10},
                 {{r0_type, TS::singleton(T_MAP)}}, {r0.map_fd == 10});
}

TEST_CASE("join map vs non-map keeps map fd", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_MAP)}}, {r0.map_fd == 7},
                 {{r0_type, TS::singleton(T_NUM)}}, {},
...
TEST_CASE("join with bottom on one side returns other", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({}, {r2.svalue == 0, r2.svalue != 0},
...
TEST_CASE("join regression from 74+103 to 104", "[join][lattice]") {
    ...
    require_join(
        {{r0_type, TS::singleton(T_MAP)}, {r6_type, TS::singleton(T_MAP)},
         {r7_type, TS::singleton(T_NUM)}, {r10_type, TS::singleton(T_STACK)}},
        {
            r0.svalue == 0,
            r6.svalue == 0,
            r7.svalue == 0,
        ...
        {{r0_type, TS::singleton(T_NUM)}, {r6_type, TS::singleton(T_NUM)},
         {r7_type, TS::singleton(T_NUM)}, {r10_type, TS::singleton(T_STACK)}},
        {
            r7.svalue >= 1,
            r1.svalue == 146,
        ...
        {{r0_type, TS::of({T_MAP, T_NUM})}, {r6_type, TS::of({T_MAP, T_NUM})},
         {r7_type, TS::singleton(T_NUM)}, {r10_type, TS::singleton(T_STACK)}},
        {
            r7.svalue >= 0,
            r1.svalue >= 146,
...
TEST_CASE("join may lose hole if other branch allows small non-negative values", "[join][lattice]") {
    ...
    require_join({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue <= -1, r0.uvalue >= two63},
                 {{r0_type, TS::singleton(T_NUM)}},
                 {r0.svalue >= 0, r0.svalue <= 100, r0.uvalue >= 0, r0.uvalue <= 100},
                 {{r0_type, TS::singleton(T_NUM)}}, {});
}
...
TEST_CASE("join does not conflate value facts across pointer/num", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_NUM)}}, {r0.svalue == 42},
...
TEST_CASE("join preserves multiple per-type fields across type widening", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_PACKET)}}, {r0.packet_offset == 8},
                 {{r0_type, TS::singleton(T_STACK)}}, {r0.stack_offset == 96},
                 {{r0_type, TS::of({T_PACKET, T_STACK})}}, {r0.packet_offset == 8, r0.stack_offset == 96});
}
...
TEST_CASE("join map fd differing constants forms hull", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({{r0_type, TS::singleton(T_MAP)}}, {r0.map_fd == 7},
                 {{r0_type, TS::singleton(T_MAP)}}, {r0.map_fd == 9},
                 {{r0_type, TS::singleton(T_MAP)}}, {r0.map_fd >= 7, r0.map_fd <= 9});
}
