...
TEST_CASE("EbpfDomain disjoint value join", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_NUM}, {r0.svalue == 0}, {r0.type == T_NUM}, {r0.svalue == 5}, {r0.type == T_NUM},
                 {r0.svalue >= 0, r0.svalue <= 5});
}

TEST_CASE("join respects partial order", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_NUM}, {r0.svalue == 1}, {r0.type == T_NUM}, {r0.svalue == 2}, {r0.type == T_NUM},
...
TEST_CASE("join keeps equal constants", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_NUM}, {r0.svalue == 7}, {r0.type == T_NUM}, {r0.svalue == 7}, {r0.type == T_NUM},
                 {r0.svalue == 7});
}

TEST_CASE("join convex hull of disjoint constants", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_NUM}, {r0.svalue == 1}, {r0.type == T_NUM}, {r0.svalue == 3}, {r0.type == T_NUM},
                 {r0.svalue >= 1, r0.svalue <= 3});
}

TEST_CASE("join overlapping intervals", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_NUM}, {r0.svalue >= 0, r0.svalue <= 5}, {r0.type == T_NUM},
                 {r0.svalue >= 3, r0.svalue <= 10}, {r0.type == T_NUM}, {r0.svalue >= 0, r0.svalue <= 10});
}
...
TEST_CASE("join same pointer type same offset", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_PACKET}, {r0.svalue == 123, r0.packet_offset == 4}, {r0.type == T_PACKET},
                 {r0.svalue == 123, r0.packet_offset == 4}, {r0.type == T_PACKET},
                 {r0.svalue == 123, r0.packet_offset == 4});
}

TEST_CASE("join ranges offsets for same pointer type", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_PACKET}, {r0.svalue == 1, r0.packet_offset == 0}, {r0.type == T_PACKET},
                 {r0.svalue == 3, r0.packet_offset == 4}, {r0.type == T_PACKET},
...
TEST_CASE("join keeps offsets across disjoint types", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r1.type == T_STACK}, {r1.svalue == 123, r1.stack_offset == 100}, {r1.type == T_PACKET},
                 {r1.svalue == 123, r1.packet_offset == 4}, {r1.type >= T_PACKET, r1.type <= T_STACK},
...
TEST_CASE("join with matching map types", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_MAP}, {r0.svalue == 1}, {r0.type == T_MAP}, {r0.svalue == 3}, {r0.type == T_MAP},
                 {r0.svalue >= 1, r0.svalue <= 3});
}
...
TEST_CASE("join with different types and unknown offsets", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_MAP}, {r0.svalue == 1}, {r0.type == T_STACK}, {r0.svalue == 2}, {r0.type <= T_STACK},
                 {r0.type >= T_MAP, r0.svalue >= 1, r0.svalue <= 2});
}

// 6) Shared memory offsets and sizes
TEST_CASE("join shared offsets and sizes", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_SHARED}, {r0.shared_offset == 16, r0.shared_region_size == 64}, {r0.type == T_SHARED},
                 {r0.shared_offset == 32, r0.shared_region_size == 64}, {r0.type == T_SHARED},
...
TEST_CASE("join preserves shared offset across type widening", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_SHARED}, {r0.shared_offset == 0},
                 {r0.type == T_NUM}, {},
...
TEST_CASE("join stack offsets and numeric size", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_STACK}, {r0.stack_offset == 64, r0.stack_numeric_size == 8}, {r0.type == T_STACK},
                 {r0.stack_offset == 96, r0.stack_numeric_size == 8}, {r0.type == T_STACK},
...
TEST_CASE("join map fd constants", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_MAP}, {r0.map_fd == 10}, {r0.type == T_MAP}, {r0.map_fd == 10}, {r0.type == T_MAP},
                 {r0.map_fd == 10});
}

TEST_CASE("join map vs non-map keeps map fd", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_MAP}, {r0.map_fd == 7}, {r0.type == T_NUM}, {}, {r0.type <= T_NUM, r0.type >= T_MAP},
                 {r0.map_fd == 7});
}
...
TEST_CASE("join with bottom on one side returns other", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({}, {r2.svalue == 0, r2.svalue != 0}, {r0.type == T_NUM}, {r2.svalue == 5}, {r0.type == T_NUM},
                 {r2.svalue == 5});
}
...
TEST_CASE("join regression from 74+103 to 104", "[join][lattice]") {
    ...
    require_join(
        {r0.type == T_MAP, r6.type == T_MAP, r7.type == T_NUM, r10.type == T_STACK},
        {
            r0.svalue == 0,
            r6.svalue == 0,
            r7.svalue == 0,
        ...
        {r0.type == T_NUM, r6.type == T_NUM, r7.type == T_NUM, r10.type == T_STACK},
        {
            r7.svalue >= 1,
            r1.svalue == 146,
            r10.svalue >= 4096,
        ...
        {r0.type >= T_MAP, r0.type <= T_NUM, r6.type >= T_MAP, r6.type <= T_NUM, r7.type == T_NUM, r10.type == T_STACK},
        {
            r7.svalue >= 0,
            r1.svalue >= 146,
            r1.svalue <= 153,
...
TEST_CASE("join drops opposing order relations but keeps per-var hulls", "[join][lattice]") {
    ...
    require_join({r0.type == T_NUM, r1.type == T_NUM},
...
TEST_CASE("join preserves svalue/uvalue-implied hole when both branches imply it", "[join][lattice]") {
    ...
    require_join({r0.type == T_NUM}, {r0.svalue <= -1, r0.uvalue >= two63}, {r0.type == T_NUM},
                 {r0.svalue <= -10, r0.uvalue >= two63}, {r0.type == T_NUM}, {r0.svalue <= -1, r0.uvalue >= two63});
}
...
TEST_CASE("join is commutative and idempotent (domain-wide)", "[join][lattice]") {
    ...
    const std::vector TB{r0.type == T_NUM};
    const std::vector B{r0.svalue >= 2, r0.svalue <= 5};
    const std::vector Tover{r0.type >= T_NUM, r0.type <= T_PACKET};
...
TEST_CASE("join convex hull with extreme 64-bit bounds", "[join][lattice]") {
    ...
    require_join({r0.type == T_NUM}, {r0.svalue >= min64, r0.svalue <= -1}, {r0.type == T_NUM},
                 {r0.svalue >= 1, r0.svalue <= max64}, {r0.type == T_NUM}, {r0.svalue >= min64, r0.svalue <= max64});
}

// 16) Disjoint intervals with a gap
TEST_CASE("join hull of disjoint intervals with gap", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_NUM}, {r0.svalue >= -10, r0.svalue <= -1}, {r0.type == T_NUM},
                 {r0.svalue >= 1, r0.svalue <= 10}, {r0.type == T_NUM}, {r0.svalue >= -10, r0.svalue <= 10});
}
...
TEST_CASE("join map fd differing constants forms hull", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join({r0.type == T_MAP}, {r0.map_fd == 7}, {r0.type == T_MAP}, {r0.map_fd == 9}, {r0.type == T_MAP},
...
TEST_CASE("join shared region size differing values", "[join][lattice]") {
    using namespace dsl_syntax;
    require_join(
        {r0.type == T_SHARED}, {r0.shared_offset == 0, r0.shared_region_size == 64}, {r0.type == T_SHARED},
        {r0.shared_offset == 32, r0.shared_region_size == 128}, {r0.type == T_SHARED},
...
TEST_CASE("join preserves multi-register per-type field", "[join][lattice]") {
    ...
    require_join({r6.type == T_PACKET}, {r6.packet_offset == 4, r7.type == T_NUM},
                 ...
                 {r6.type >= T_NUM, r6.type <= T_PACKET, r7.type >= T_NUM, r7.type <= T_STACK},
