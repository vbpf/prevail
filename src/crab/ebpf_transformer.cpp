...
class EbpfTransformer final {
  ...
  private:
    ...
    void recompute_stack_numeric_size(NumAbsDomain& inv, const ProgVar& type_variable) const;
...
void EbpfTransformer::save_callee_saved_registers(const std::string& prefix) {
    ...
    for (int r = R6; r <= R9; r++) {
        for (const DataKind kind : iterate_kinds()) {
            const ProgVar src_var = variable_registry->reg(kind, r);
...
void EbpfTransformer::restore_callee_saved_registers(const std::string& prefix) {
    for (int r = R6; r <= R9; r++) {
        for (const DataKind kind : iterate_kinds()) {
            const ProgVar src_var = variable_registry->stack_frame_var(kind, r, prefix);
            if (!m_inv.eval_interval(src_var).is_top()) {
...
void EbpfTransformer::forget_packet_pointers() {
    ...
    for (const ProgVar type_variable : variable_registry->get_type_variables()) {
 ...
 */
static LinearConstraint assume_cst_offsets_reg(const Condition::Op op, const ProgVar& dst_offset,
                                               const ProgVar& src_offset) {
    using namespace dsl_syntax;
...
void EbpfTransformer::operator()(const Un& stmt) {
    ...
    auto swap_endianness = [&](const ProgVar& v, auto be_or_le) {
...
void EbpfTransformer::do_load_stack(NumAbsDomain& inv, const Reg& target_reg, const LinearExpression& addr,
    ...
    if (width == 1 || width == 2 || width == 4 || width == 8) {
        ...
        if (type_inv.may_have_type(inv, target.type, T_MAP) ||
            type_inv.may_have_type(inv, target.type, T_MAP_PROGRAMS)) {
...
void EbpfTransformer::do_store_stack(NumAbsDomain& inv, const LinearExpression& addr, const int width,
                                     ...
                                     const LinearExpression& val_uvalue, const std::optional<RegPack>& opt_val_reg) {
    {
        const std::optional<ProgVar> var = stack.store_type(inv, addr, width, val_type);
    ...
    if (width == 8) {
        ...
        if (opt_val_reg && (type_inv.may_have_type(m_inv, val_type, T_MAP) ||
                            type_inv.may_have_type(m_inv, val_type, T_MAP_PROGRAMS))) {
    ...
    for (const ProgVar type_variable : variable_registry->get_type_variables()) {
        ...
        const ProgVar stack_offset_variable = variable_registry->kind_var(DataKind::stack_offsets, type_variable);
        const ProgVar stack_numeric_size_variable =
...
void EbpfTransformer::operator()(const Call& call) {
    ...
    for (ArgPair param : call.pairs) {
        ...
        case ArgPair::Kind::PTR_TO_WRITABLE_MEM: {
            ...
            ProgVar addr = variable.value();
            ProgVar width = reg_pack(param.size).svalue;
...
void EbpfTransformer::recompute_stack_numeric_size(NumAbsDomain& inv, const ProgVar& type_variable) const {
    const ProgVar stack_numeric_size_variable =
        variable_registry->kind_var(DataKind::stack_numeric_sizes, type_variable);
