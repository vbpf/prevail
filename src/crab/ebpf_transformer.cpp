...
namespace prevail {
    ...
    public:
        explicit EbpfTransformer(EbpfDomain &dom) : dom(dom), m_inv(dom.m_inv), stack(dom.stack),
    ...
    private:
        ...
        void do_load_packet_or_shared(NumAbsDomain &inv, const Reg &target_reg, const LinearExpression &addr,
                                      int width);

        void do_load(const Mem &b, const Reg &target_reg);

        void do_store_stack(NumAbsDomain &inv, const LinearExpression &addr, int width,
    ...
    void EbpfTransformer::do_load_stack(NumAbsDomain &inv, const Reg &target_reg, const LinearExpression &addr,
        ...
        if (width == 1 || width == 2 || width == 4 || width == 8) {
            ...
            if (type_inv.may_have_type(inv, target.type, T_MAP) || type_inv.may_have_type(
                    inv, target.type, T_MAP_PROGRAMS)) {
    ...
    void EbpfTransformer::do_load_packet_or_shared(NumAbsDomain &inv, const Reg &target_reg,
                                                   const LinearExpression &addr,
                                                   const int width) {
        if (inv.is_bottom()) {
            return;
        }
    ...
    void EbpfTransformer::do_store_stack(NumAbsDomain &inv, const LinearExpression &addr, const int width,
                                         ...
                                         const LinearExpression &val_uvalue,
                                         const std::optional<RegPack> &opt_val_reg) { {
        ...
        if (width == 8) {
            ...
            if (opt_val_reg &&
                (type_inv.may_have_type(m_inv, val_type, T_MAP) || type_inv.may_have_type(
                     m_inv, val_type, T_MAP_PROGRAMS))) {
            ...
            if (opt_val_reg && type_inv.may_have_type(m_inv, val_type, T_SHARED)) {
                ...
                inv.assign(
                    stack.store(inv, DataKind::shared_region_sizes, addr, width, opt_val_reg->shared_region_size),
            ...
            if (opt_val_reg && type_inv.may_have_type(m_inv, val_type, T_STACK)) {
                ...
                inv.assign(
                    stack.store(inv, DataKind::stack_numeric_sizes, addr, width, opt_val_reg->stack_numeric_size),
    ...
    void EbpfTransformer::do_mem_store(const Mem &b, const LinearExpression &val_type,
                                       const LinearExpression &val_svalue,
                                       const LinearExpression &val_uvalue, const std::optional<RegPack> &opt_val_reg) {
        if (m_inv.is_bottom()) {
            return;
        }
    ...
    static Bin atomic_to_bin(const Atomic &a) {
        ...
        switch (a.op) {
            case Atomic::Op::ADD: bin.op = Bin::Op::ADD;
                break;
            case Atomic::Op::OR: bin.op = Bin::Op::OR;
                break;
            case Atomic::Op::AND: bin.op = Bin::Op::AND;
                break;
            case Atomic::Op::XOR: bin.op = Bin::Op::XOR;
                break;
            case Atomic::Op::XCHG:
            case Atomic::Op::CMPXCHG: bin.op = Bin::Op::MOV;
                break;
    ...
    void EbpfTransformer::operator()(const Call &call) {
        ...
        for (ArgSingle param: call.singles) {
            switch (param.kind) {
                case ArgSingle::Kind::MAP_FD: maybe_fd_reg = param.reg;
                    break;
        ...
        if (call.is_map_lookup) {
            ...
            if (maybe_fd_reg) {
                if (const auto map_type = dom.get_map_type(*maybe_fd_reg)) {
                    if (thread_local_program_info->platform->get_map_type(*map_type).value_type ==
        ...
        if (auto pimm = std::get_if<Imm>(&bin.v)) {
            ...
            switch (bin.op) {
                ...
                case Bin::Op::AND:
                    ...
                    havoc_offsets(bin.dst);
                    break;
                case Bin::Op::LSH: shl(bin.dst, gsl::narrow<int32_t>(imm), finite_width);
                    break;
                case Bin::Op::RSH: lshr(bin.dst, gsl::narrow<int32_t>(imm), finite_width);
                    break;
                case Bin::Op::ARSH: ashr(bin.dst, gsl::narrow<int32_t>(imm), finite_width);
                    break;
                case Bin::Op::XOR:
            ...
            switch (bin.op) {
                case Bin::Op::ADD: {
                    if (type_inv.same_type(m_inv, bin.dst, std::get<Reg>(bin.v))) {
                        ...
                        m_inv = type_inv.join_over_types(m_inv, bin.dst,
                                                         [&](NumAbsDomain &inv, const TypeEncoding dst_type) {
                                                             inv = type_inv.join_over_types(
                                                                 inv, src_reg,
                                                                 [&](NumAbsDomain &inv, const TypeEncoding src_type) {
                                                                     if (dst_type == T_NUM && src_type != T_NUM) {
                                                                         // num += ptr
                                                                         type_inv.assign_type(inv, bin.dst, src_type);
                                                                         if (const auto dst_offset = dom.
                                                                                 get_type_offset_variable(
                                                                                     bin.dst, src_type)) {
                                                                             inv->apply(ArithBinOp::ADD,
                                                                                 dst_offset.value(), dst.svalue,
                                                                                 dom.get_type_offset_variable(
                                                                                     src_reg, src_type).value());
                                                                         }
                                                                         if (src_type == T_SHARED) {
                                                                             inv.assign(dst.shared_region_size,
                                                                                 src.shared_region_size);
                                                                         }
                                                                     } else if (dst_type != T_NUM && src_type ==
                                                                         T_NUM) {
                                                                         // ptr += num
                                                                         ...
                                                                         if (const auto dst_offset = dom.
                                                                                 get_type_offset_variable(
                                                                                     bin.dst, dst_type)) {
                                                                             inv->apply(ArithBinOp::ADD,
                                                                                 dst_offset.value(), dst_offset.value(),
                                                                                 src.svalue);
                                                                             if (dst_type == T_STACK) {
                                                                         ...
                                                                         inv->apply_signed(
                                                                             ArithBinOp::ADD, dst.svalue, dst.uvalue,
                                                                             dst.svalue, src.svalue,
                                                                             finite_width);
                                                                     } else {
                        ...
                        m_inv->apply_signed(ArithBinOp::ADD, dst.svalue, dst.uvalue, dst.svalue, src.svalue,
                                            finite_width);
                    }
                    break;
                }
                case Bin::Op::SUB: {
                    if (type_inv.same_type(m_inv, bin.dst, std::get<Reg>(bin.v))) {
                        ...
                        m_inv = type_inv.join_over_types(m_inv, bin.dst,
                                                         [&](NumAbsDomain &inv, const TypeEncoding type) {
                                                             switch (type) {
                                                                 case T_NUM:
                                                                     // This is: sub_overflow(inv, dst.value, src.value, finite_width);
                                                                     inv->apply_signed(
                                                                         ArithBinOp::SUB, dst.svalue, dst.uvalue,
                                                                         dst.svalue, src.svalue,
                                                                         finite_width);
                                                                 ...
                                                                 default:
                                                                     ...
                                                                     if (const auto dst_offset = dom.
                                                                             get_type_offset_variable(bin.dst, type)) {
                                                                         inv->apply_signed(
                                                                             ArithBinOp::SUB, dst.svalue, dst.uvalue,
                                                                             dst_offset.value(),
                                                                             dom.get_type_offset_variable(src_reg, type)
                                                                             .value(), finite_width);
                ...
                case Bin::Op::MOVSX32: {
                    ...
                    if (dst.svalue == src.svalue && m_inv.eval_interval(dst.svalue) <= Interval::signed_int(
                            source_width)) {
                        return;
                    }
                    if (m_inv.entail(type_is_number(src_reg))) {
                    ...
                    m_inv = type_inv.join_over_types(m_inv, src_reg, [&](NumAbsDomain &inv, const TypeEncoding type) {
                        switch (type) {
                            ...
                            case T_STACK:
                                ...
                                break;
                            default: inv.assign(dst.type, type);
                                break;
                        }
                    });
                    if (bin.is64) {
