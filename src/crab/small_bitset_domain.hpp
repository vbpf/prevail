...
class SmallBitsetDomain {
  ...
  public:
    ...
    // Bit access
    [[nodiscard]] constexpr bool test(unsigned i) const { return (bits_ & (UInt{1} << i)) != 0; }
    ...
    [[nodiscard]] constexpr bool is_singleton() const { return bits_ != 0 && (bits_ & (bits_ - 1)) == 0; }
    [[nodiscard]] int count() const { return __builtin_popcountll(static_cast<unsigned long long>(bits_)); }

    /// The underlying integer value.
    [[nodiscard]] constexpr UInt raw() const { return bits_; }
};
