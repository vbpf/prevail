...
static json handle_get_slice(const json& args, AnalysisEngine& engine) {
    ...
    const std::string program = args.value("program", "");
    const std::string type = args.value("program_type", "");
    const size_t trace_depth = args.value("trace_depth", 200);
    const auto opt_termination = args.contains("check_termination")
                                     ? std::optional<bool>(args["check_termination"].get<bool>())
                                     : std::nullopt;
    const auto opt_div_zero = args.contains("allow_division_by_zero")
    ...
    // Use backward slicing for both error queries and arbitrary PC queries.
    // For errors: compute_failure_slices seeds from the failing assertion's registers.
    // For arbitrary PCs: compute_slice_from_label seeds from the instruction's read registers.
    try {
        if (is_error_query && session.failed) {
            auto slices =
                engine.compute_failure_slices(elf_path, section, program, type, session.program, 1, trace_depth);
            if (!slices.empty()) {
                j["failure_slice"] = serialize_slice(slices.front(), session, target_label);
                return j;
            }
        }
        ...
        std::cerr << "prevail_mcp: slicing failed: " << e.what() << std::endl;
...
static prevail::InstructionSeq parse_assembly(const std::string& code, const prevail::ebpf_platform_t* platform) {
    ...
    int label_index = 0;
    std::regex call_regex(R"(call\s+(\d+).*)");
    for (const auto& block : blocks) {
         ...
         {"properties",
          {{"elf_path", {{"type", "string"}}},
           ...
           {"trace_depth",
            {{"type", "integer"},
             {"default", 200},
             {"description", "Maximum backward steps for slicing (default: 200, matching check.exe)"}}},
           {"section", {{"type", "string"}}},
           {"program", {{"type", "string"}}},
           {"program_type",
            {{"type", "string"}, {"description", "Program type name override (e.g. \"xdp\", \"bind\")"}}},
